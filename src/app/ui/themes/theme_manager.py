...
class ThemeManager:
    ...
    def _get_settings_file_path(self) -> str:
        ...
        try:
            return get_user_data_path("player_data.json")
        except Exception as e:
            logging.error(f"Error getting theme settings path: {e}")
            return "player_data.json"  # Fallback to current directory
            ...
            if os.path.exists(self.settings_file):
                with open(self.settings_file, "r") as f:
                    ...
                    saved_theme = (
                        data.get("settings", {})
                        .get("theme", {})
                        .get("current_theme", get_default_theme())
                    )
                    ...
                    if saved_theme in AVAILABLE_THEMES:
                        ...
                        logging.warning(
                            f"Invalid saved theme '{saved_theme}', using default"
                        )
                        self.current_theme_name = get_default_theme()
        except Exception as e:
    ...
    def _save_theme_preference(self):
        ...
        try:
            ...
            if "theme" not in data["settings"]:
                data["settings"]["theme"] = {}
            data["settings"]["theme"]["current_theme"] = self.current_theme_name
    ...
    def _notify_theme_change(self, old_theme: str, new_theme: str):
        ...
        for callback in self.theme_change_callbacks:
            try:
                ...
                logging.error(
                    f"Error in theme change callback {callback.__name__}: {e}"
                )
