...
class DataService:
            ...
            if "IdentityToken" in message:
                ...
                if claim_check:
                    claim_list = sorted(
                        [
                            c["name"]
                            for c in self.app.tables["claim_state"]
                            if "name" in c
                        ]
                    )
    ...
    def refresh_global_prices(self):
        ...
        self.global_product_queue = sorted(
            self.app.product_rost,
    ...
    def check_global_queue(self):
        ...
        self.global_product_queue = [
            p for p in self.global_product_queue if p != product_id
        ]
    ...
    def refresh_claim_prices(self):
        ...
        self.claim_product_queue = sorted(
            self.app.product_rost,
            key=lambda p: self.app.product_rost[p].get("Unit Price", 1e-15),
            reverse=True,
        )
    ...
    def check_claim_queue(self):
        ...
        # Calculate price and load into table
        orders = pricing.price_calc(
            app=self.app, product_id=product_id, claim_id=claim_id
        )
        P_e = orders["P_e"]
        ratio = self.app.product_rost.get(product_id, {}).get("Pack Size", 1)
        pack_price = np.round(ratio * P_e, 1)
        ...
        # Remove all copies of completed product id from queue
        self.claim_product_queue = [
            p for p in self.claim_product_queue if p != product_id
        ]
