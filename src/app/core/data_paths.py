"""
Data path utilities for BitCraft Econ Console.
...
def get_bundled_data_directory():
    ...
    if getattr(sys, "frozen", False):
        ...
        return os.path.join(
            getattr(sys, "_MEIPASS", os.path.dirname(sys.executable)), "data"
        )
    else:
    ...
    try:
        ...
        if os.path.exists(source_path):
            try:
                ...
                logging.info(
                    f"Migrated {filename} from {app_dir} to user data directory: {target_path}"
                )
                ...
                try:
                    ...
                    logging.info(
                        f"Copied {filename} from {app_dir} to user data directory as fallback"
                    )
                except Exception as copy_error:
                    logging.error(
                        f"Failed to copy {filename} as fallback: {copy_error}"
                    )
        else:
            logging.info(
